...
function TimeUnit({ value, label, delay = 0 }: { value: number; label: string; delay?: number }) {
  ...
  return (
    ...
    >
      <motion.div
        key={displayValue}
        className="relative bg-gradient-to-br from-[#7C3AED] to-[#8B5CF6] text-white rounded-xl w-12 h-12 flex items-center justify-center shadow-lg shadow-purple-200"
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        >
          {isStarted ? (
            ...
            <Timer className={`w-5 h-5 ${isUrgent ? "text-orange-500" : "text-[#7C3AED]"}`} />
          )}
        </motion.div>
